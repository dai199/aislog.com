Setlist.seed(:id,
  {
    id: 1,
    title: "AIS-Friend vol.4 〜with 原田珠々華さん〜",
    event_date: Time.zone.parse('2018-09-02 17:00'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 2,
    title: "アイドル無双（1部）〜AIS 橋本麗愛＆関澤朋花 凱旋公演〜",
    event_date: Time.zone.parse('2018-09-01 11:00'),
    place: "郡山 HIPSHOT JAPAN",
    description: ""
  },
  {
    id: 3,
    title: "アイドル無双（2部）",
    event_date: Time.zone.parse("2018-09-01 16:30"),
    place: "郡山 HIPSHOT JAPAN",
    description: ""
  },
  {
    id: 4,
    title: "@JAM EXPO 2018",
    event_date: Time.zone.parse("2018-08-25 11:35"),
    place: "横浜アリーナ",
    description: ""
  },
  {
    id: 5,
    title: "@JAM EXPO 2018",
    event_date: Time.zone.parse("2018-08-25 16:20"),
    place: "横浜アリーナ",
    description: ""
  },
  {
    id: 6,
    title: "いたばCフェス2018 〜板橋加奈 Birthday Party〜",
    event_date: Time.zone.parse("2018-08-19 12:30"),
    place: "Future SEVEN(東京・南青山)",
    description: ""
  },
  {
    id: 7,
    title: 'AIS-Friend（アイスフレンド）vol.3 ～with パンダみっくさん～',
    event_date: Time.zone.parse('2018-08-18 11:30'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 8,
    title: "AIS-Scream（アイスクリーム）特別公演 〜最近やってない曲、だいたいやります〜",
    event_date: Time.zone.parse('2018-08-18 17:30'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 9,
    title: "ばぶる★らっぷ Vol.2",
    event_date: Time.zone.parse('2018-08-12 16:30'),
    place: "東京都 TSUTAYA O-nest",
    description: ""
  },
  {
    id: 10,
    title: "IDOL Pop'n Party",
    event_date: Time.zone.parse('2018-08-11 12:00'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 11,
    title: "TOKYO IDOL FESTIVAL 2018",
    event_date: Time.zone.parse('2018-08-04 11:20'),
    place: "FUJI YOKO STAGE",
    description: ""
  },
  {
    id: 12,
    title: "TOKYO IDOL FESTIVAL 2018",
    event_date: Time.zone.parse('2018-08-04 15:40'),
    place: "FESTIVAL STAGE",
    description: ""
  },
  {
    id: 13,
    title: "TOKYO IDOL FESTIVAL 2018",
    event_date: Time.zone.parse('2018-08-03 18:35'),
    place: "SKY STAGE",
    description: ""
  },
  {
    id: 14,
    title: "夏の天使かよ！～天使かよ！2周年スペシャル〜in ZeppTokyo",
    event_date: Time.zone.parse('2018-07-25 17:35'),
    place: "Zepp Tokyo",
    description: ""
  },
  {
    id: 15,
    title: "「NEO Fes!!! presented by Top Yell」 Vol.5",
    event_date: Time.zone.parse('2018-07-16 11:30'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 16,
    title: "BLC LIVE PREMIUM VOL.5",
    event_date: Time.zone.parse('2018-07-14 11:50'),
    place: "X-HALL-ZEN-",
    description: ""
  },
  {
    id: 17,
    title: "BLC LIVE VOL.33",
    event_date: Time.zone.parse('2018-07-14 15:40'),
    place: "伏見ライオンシアター",
    description: ""
  },
  {
    id: 18,
    title: "アイドル横丁夏まつり!!〜2018〜",
    event_date: Time.zone.parse('2018-07-08 13:45'),
    place: "1番地ステージ",
    description: ""
  },
  {
    id: 19,
    title: "アイドル横丁夏まつり!!〜2018〜",
    event_date: Time.zone.parse('2018-07-07 11:20'),
    place: "4番地ステージ",
    description: ""
  },
  {
    id: 20,
    title: "アイドル横丁夏まつり!!〜2018〜",
    event_date: Time.zone.parse('2018-07-07 16:10'),
    place: "2番地ステージ",
    description: ""
  },
  {
    id: 21,
    title: "IDOL Pop'n Party",
    event_date: Time.zone.parse('2018-06-24 12:00'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 22,
    title: "DDD~Discovery iDol Depot~",
    event_date: Time.zone.parse('2018-06-24 17:15'),
    place: "白金高輪SELENE b2",
    description: ""
  },
  {
    id: 23,
    title: "AIS-Carnival（アイスカーニバル）vol.3 〜AISの夏がやってくる！お披露目2周年公演〜",
    event_date: Time.zone.parse('2018-06-23 11:30'),
    place: "新宿 Zirco Tokyo",
    description: ""
  },
  {
    id: 24,
    title: "なんてったってハロプロライブ スペシャル 1部",
    event_date: Time.zone.parse('2018-06-17 12:00'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 25,
    title: "なんてったってハロプロライブ スペシャル 2部",
    event_date: Time.zone.parse('2018-06-17 18:30'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 26,
    title: "IDOL Pop'n Party",
    event_date: Time.zone.parse('2018-06-02 11:30'),
    place: "渋谷 Clubasia",
    description: ""
  },
  {
    id: 27,
    title: "東京アイドル劇場アドバンス",
    event_date: Time.zone.parse('2018-05-20 15:15'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 28,
    title: "「アイドル無双」足利五月の陣 1部",
    event_date: Time.zone.parse('2018-05-19 14:20'),
    place: "足利ライブハウス大使館",
    description: ""
  },
  {
    id: 29,
    title: "「アイドル無双」足利五月の陣 2部",
    event_date: Time.zone.parse('2018-05-19 18:15'),
    place: "足利ライブハウス大使館",
    description: ""
  },
  {
    id: 30,
    title: "AIS-Carnival（アイスカーニバル）vol.2 〜アコースティックで結成2周年〜",
    event_date: Time.zone.parse('2018-05-05 12:00'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 31,
    title: "DDD〜Discovery iDol Depot〜",
    event_date: Time.zone.parse('2018-05-05 17:15'),
    place: "白金高輪SELENE b2",
    description: ""
  },
  {
    id: 32,
    title: "めっちゃたのしいフェス2018 〜GWだ！そうだ！お外で！アイドル！見よう！！〜",
    event_date: Time.zone.parse('2018-05-04 13:00'),
    place: "上野恩賜公園野外ステージ",
    description: ""
  },
  {
    id: 33,
    title: "東京アイドル劇場アドバンス",
    event_date: Time.zone.parse('2018-04-22 13:50'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 34,
    title: "レコチョク presents BLITZの天使かよ！ supported by WIZY",
    event_date: Time.zone.parse('2018-04-21 13:00'),
    place: "マイナビBLITZ赤坂",
    description: ""
  },
  {
    id: 35,
    title: "AIS-Carnival（アイスカーニバル）vol.1",
    event_date: Time.zone.parse('2018-04-14 17:30'),
    place: "白金高輪SELENE b2",
    description: ""
  },
  {
    id: 36,
    title: "私たち、正統派。Vol.26 ～渋谷TAKE OFF7 スペシャル～",
    event_date: Time.zone.parse('2018-04-08 11:00'),
    place: "渋谷TAKE OFF7",
    description: ""
  },
  {
    id: 37,
    title: "AIS-Cake（アイスケーキ）特別編 〜なんてったってアイドルをマスター！〜",
    event_date: Time.zone.parse('2018-04-07 12:00'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 38,
    title: "KKBOX presents 俺たち、楽曲派。Vol.27",
    event_date: Time.zone.parse('2018-04-06 17:00'),
    place: "渋谷クラブクアトロ ",
    description: ""
  },
  {
    id: 39,
    title: "俺たち、パフォ厨。Vol.3",
    event_date: Time.zone.parse('2018-03-31 12:00'),
    place: "渋谷SPACE ODD",
    description: ""
  },
  {
    id: 40,
    title: "AIS-Snack（アイスナック） 特別編～500円で、こいしょ!!!～",
    event_date: Time.zone.parse('2018-03-24 14:00'),
    place: "フジさんのヨコ",
    description: ""
  },
  {
    id: 41,
    title: "AIS-Scream（アイスクリーム）特別公演 〜あざやかな未来へ〜",
    event_date: Time.zone.parse('2018-03-24 17:00'),
    place: "フジさんのヨコ",
    description: ""
  },
  {
    id: 42,
    title: "なんてったってハロプロライブ vol.21",
    event_date: Time.zone.parse('2018-03-20 19:00'),
    place: "TwinBox GARAGE",
    description: ""
  },
  {
    id: 43,
    title: "DDD~Discovery iDol Depot~",
    event_date: Time.zone.parse('2018-03-17 11:20'),
    place: "白金高輪SELENE b2",
    description: ""
  },
  {
    id: 44,
    title: "IDOL Pop'n Party Vol.36",
    event_date: Time.zone.parse('2018-03-11 11:30'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 45,
    title: "2マン公演AIS-Friend（アイスフレンド）vol.2 〜with amiinAさん〜",
    event_date: Time.zone.parse('2018-03-10 18:00'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 46,
    title: "俺たち、パフォ厨。Vol.2",
    event_date: Time.zone.parse('2018-02-25 12:00'),
    place: "渋谷SPACE ODD",
    description: ""
  },
  {
    id: 47,
    title: "IDOL Pop’n Party Vol.35",
    event_date: Time.zone.parse('2018-02-25 17:05'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 48,
    title: "私たち、正統派。Vol.22～フジさんのヨコスペシャル～",
    event_date: Time.zone.parse('2018-02-17 12:00'),
    place: "フジさんのヨコ",
    description: ""
  },
  {
    id: 49,
    title: "ミニ公演AIS-Snack（アイスナック）vol.2 〜バレンタイン魂〜",
    event_date: Time.zone.parse('2018-02-17 17:00'),
    place: "フジさんのヨコ",
    description: ""
  },
  {
    id: 50,
    title: "ミニ公演AIS-Snack（アイスナック）vol.1 〜ノンストップで行ってみましょ♪〜",
    event_date: Time.zone.parse('2018-01-28 12:30'),
    place: "フジさんのヨコ",
    description: ""
  },
  {
    id: 51,
    title: "私たち、正統派。Vol.19　～フジさんのヨコスペシャル～",
    event_date: Time.zone.parse('2018-01-28 16:00'),
    place: "フジさんのヨコ",
    description: ""
  },
  {
    id: 52,
    title: "大好きな君に会いにいく〜TOKYO FM HALL編〜",
    event_date: Time.zone.parse('2018-01-14 11:00'),
    place: "TOKYO FM HALL",
    description: ""
  },
  {
    id: 53,
    title: "AIS-Friend（アイスフレンド）vol.1〜with〜ハコイリ♡ムスメ",
    event_date: Time.zone.parse('2018-01-13 18:00'),
    place: "AKIBAカルチャーズ劇場",
    description: ""
  },
  {
    id: 54,
    title: "アイドル横丁大忘年会!!～2017円SP!!～",
    event_date: Time.zone.parse('2017-12-30 12:20'),
    place: "ZeppTokyo",
    description: ""
  },
  {
    id: 55,
    title: "DDD 2017 iDol Summary",
    event_date: Time.zone.parse('2017-12-30 15:55'),
    place: "白金高輪SELENE b2",
    description: ""
  },
  {
    id: 56,
    title: "IDOL Pop’n Party vol.31",
    event_date: Time.zone.parse('2017-12-28 14:00'),
    place: "渋谷SPACE ODD",
    description: ""
  },
  {
    id: 57,
    title: "AIS-Cake（アイスケーキ）vol.11 〜マジ グッドチャンス クリスマス〜",
    event_date: Time.zone.parse('2017-12-25 18:30'),
    place: "J-SQUARE SHINAGAWA",
    description: ""
  },
  {
    id: 58,
    title: "DDD~Discovery iDol Xmas~",
    event_date: Time.zone.parse('2017-12-24 11:30'),
    place: "白金高輪SELENE b2",
    description: ""
  },
  {
    id: 59,
    title: "ライブハウス・R-1_U18スペシャル",
    event_date: Time.zone.parse('2017-12-23 17:05'),
    place: "NHK放送センター509",
    description: ""
  },
  {
    id: 60,
    title: "クリスマスツリーの日に品川かよ！",
    event_date: Time.zone.parse('2017-12-07 19:40'),
    place: "J-SQUARE SHINAGAWA",
    description: "朝熊萌、島崎友莉亜、徳久陽日、磯前星来の4名で出演"
  },
  # {
  #   id: ,
  #   title: "",
  #   event_date: Time.zone.parse(''),
  #   place: "",
  #   description: ""
  # },
)